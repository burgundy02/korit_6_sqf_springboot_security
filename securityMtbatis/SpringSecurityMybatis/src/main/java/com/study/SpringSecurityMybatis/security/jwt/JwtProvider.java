...
public class JwtProvider {
    ...
    public JwtProvider(@Value("${jwt.secret}") String secret) {
        this.key = Keys.hmacShaKeyFor(Decoders.BASE64.decode(secret)); // key라는 객체를 만들기 위해선
    }
...
//        Date expirDate = new Date(new Date().getTime() + (1000l * 60 * 60 * 24 * 30));
    ...
    public String removeBearer(String bearerToken) throws RuntimeException {
        ...
        int bearerLength = "bearer ".length(); // 7은 이 문자열의 길이이다.
